	...
	/*private BigDecimal totalAmount() {
		...
		for(int i=0; i< customer.getSize(); i++) {
			for (Product product : inventory.list()) {
				temp = BigDecimal.ONE;
				tempMul = BigDecimal.ZERO;
				if (scan(customer.list().get(i).getUpc())) {
					temp = temp.multiply(new BigDecimal(product.getQuantity()));
					tempMul = temp.multiply(product.getRePrice());
				}
				total = total.add(tempMul);
			}
		}
		return total;
	}*/
